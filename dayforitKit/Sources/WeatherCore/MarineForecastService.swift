...
public struct MarineForecastService: Sendable {
    ...
    public init(
        ...
        waveProvider: WaveProvider = QueenslandWaveDataProvider(),
    ...
    public func fetchSevenDayForecast(request: MarineForecastRequest) async throws -> MarineForecastOutput {
        ...
        async let waveForecastsResult: [WaveForecast]? = try? await providers.waveProvider?.fetchWaveForecast(
            location: boatingLocation,
            days: request.forecastDays
        )
        async let waveObservationsResult: [WaveObservation]? = try? await providers.waveProvider?.fetchWaveObservation(
            location: boatingLocation
        )
        ...
        let waveForecasts = await waveForecastsResult ?? []
        let waveObservations = await waveObservationsResult ?? []
