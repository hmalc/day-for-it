...
private enum QueenslandTideStationManifest {
    ...
    static func nearest(latitude: Double, longitude: Double) -> NearestMatch? {
        guard isCovered(latitude: latitude, longitude: longitude) else { return nil }
    ...
    private static func isCovered(latitude: Double, longitude: Double) -> Bool {
        let insideBroadQueenslandBounds = (-29.5 ... -9.0).contains(latitude) && (137.5 ... 154.5).contains(longitude)
        let southOfCoastalBorder = latitude < -28.25 && longitude > 151.0
